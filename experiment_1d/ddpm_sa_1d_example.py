...
for it in range(max_iter):
    ...
    optm.zero_grad()
    # Get batch
    idx = np.random.choice(traj_split.shape[0]//x_0.shape[0],batch_size)
...
from diffusion import eval_ddpm_1d_cond

# Evaluate
n_sample = 100

step_list_to_append = np.linspace(0,999,10).astype(np.int64)
cond_label = th.repeat_interleave(label_split,25,dim=0).float()
x_t_list = eval_ddpm_1d_cond(model,dc,n_sample,traj_split,step_list_to_append,device, cond=cond_label)
...
fig.legend()
fig.savefig(outdir+'result_cond.png')