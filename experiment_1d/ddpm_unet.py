...
def main(args):
    ...
    output = f'./result/1d_example_result_{args.name}'
    ...
    # times, x_0, _, true_length, real_param = get_1d_training_data(
    ...
    # np.save('./gp1_one_ds10000.npy', dic)
    ...
    if args.train:
        ...
        for it in range(max_iter):
            ...
            # Get batch
            idx = np.random.choice(x_0.shape[0]//x_0.shape[0],batch_size)
            ...
            if args.true:
                ...
                noise_pred,_ = model(x_t_batch,step_batch,true_length_batch)    
            
            else:
                if args.cond:
                    noise_pred,_ = model(x_t_batch, step_batch, cond_batch)
                else:
            ...
            if (it%eval_every) == 0 or it == (max_iter-1):
                ...
                if args.d_type == 'gp1' or args.d_type == 'gp2':
                    ...
                    for hyp_len, slow_hyp_len in zip(len_param, slow_len_param):
                        n_sample = 20
                        leng = 0
                        if args.true:
                            leng = th.Tensor([128]*n_sample).to(device)
    ...
    if args.test:
        ...
        for hyp_len in len_param:
            for slow_hyp_len in slow_len_param:
                ...
                plot_ddpm_2d_data_result(
                    len_param=(hyp_len, slow_hyp_len), output=output, real_param=real_param,
                    times=times,x_data=x_0,step_list=step_list_to_append,x_t_list=x_t_list,
                    plot_ancestral_sampling=False,plot_one_sample=False,
                    lw_gt=2,lw_sample=1/2,lc_gt=(0,1,0,0.3),lc_sample='k',
