...
def gmm_forward(pi,mu,sigma,data=None):
    """
    ...
    out = {
        ...
        'probs':probs,'log_probs':log_probs,'nlls':nlls,
        'argmax_mu':argmax_mu
    }
    return out
...
class MixtureDensityNetwork(nn.Module):
    def __init__(
        ...
        for h_dim in self.h_dim_list:
            ...
            self.layer_list.append(nn.Dropout(p=self.p_drop))
            h_dim_prev = h_dim
...
class MixtureDensityNetworkClassifier_wo_T(nn.Module):
    def __init__(
        self,
        name       = 'mdn',
        x_dim      = 30,
        ...
        self.proj = nn.Sequential(
            nn.Linear(in_features=self.length, out_features=self.length*4),
            nn.GELU(),
            nn.LayerNorm(length*4),
            nn.Linear(in_features=self.length*4, out_features=self.length)
            ).to(self.device)
    ...
    def forward(self, x, label):
        ...
        pi_batch,mu_batch,sigma_batch = self.mdn(x)
        label = label[:, None]
        x = gmm_forward(pi_batch,mu_batch,sigma_batch, label)
        return x

class MixtureDensityNetworkClassifier_w_T(nn.Module):
    def __init__(
        self,
        name       = 'mdn',
        x_dim      = 1,
        ...
        self.proj = nn.Sequential(
            nn.Linear(in_features=self.x_dim, out_features=self.x_dim*4),
            nn.GELU(),
            nn.LayerNorm(y_dim*4),
            nn.Linear(in_features=self.x_dim*4, out_features=self.x_dim)
            ).to(self.device)
        
        
        self.dense_for_trj = nn.Sequential(
            nn.Linear(in_features=y_dim,out_features=y_dim*4),
            nn.GELU(),
            nn.LayerNorm(y_dim*4),
            nn.Linear(in_features=y_dim*4,out_features=y_dim*4),
            nn.GELU(),
            nn.LayerNorm(y_dim*4),
            nn.Linear(in_features=y_dim*4,out_features=10), # length parameter 10개
        ).to(self.device)
        
        self.dense_for_t = nn.Sequential(
            nn.Linear(in_features=y_dim,out_features=y_dim*4),
            nn.GELU(),
            nn.LayerNorm(y_dim*4),
            nn.Linear(in_features=y_dim*4,out_features=y_dim*4),
            nn.GELU(),
            nn.LayerNorm(y_dim*4),
            nn.Linear(in_features=y_dim*4,out_features=1000), # timestep 1000개 중 1개
        ).to(self.device)
    ...
    def forward(self, x, t):
        ...
        x = self.proj(x, t)
        pi_batch,mu_batch,sigma_batch = self.mdn(x)
        x = gmm_forward(pi_batch,mu_batch,sigma_batch,y_batch)['argmax_mu']
        print(x.shape)
        exit()
        trj = self.dense_for_trj(x) # [B x 10]
        t = self.dense_for_t(x) # [B]
        return trj, t
