...
class TimestepEmbedSequential2(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, bcond=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x, bcond = layer(x, emb, bcond)
            else:
                x = layer(x)
        return x, bcond
...
class ResBlock2(TimestepBlock):
    ...
    def __init__(
        ...
        self.updown_cond = self.upsample or self.downsample
        if self.upsample:
            self.h_upd_cond = Upsample(
                n_channels = self.n_channels,
                up_rate    = self.up_rate,
                up_mode    = self.sample_mode,
                dims       = self.dims)
            self.cond_upd = Upsample(
                n_channels = self.n_channels,
                up_rate    = self.up_rate,
                up_mode    = self.sample_mode,
                dims       = self.dims)
        elif self.downsample:
            self.h_upd_cond = Downsample(
                n_channels = self.n_channels,
                down_rate  = self.down_rate,
                dims       = self.dims)
            self.cond_upd = Downsample(
                n_channels = self.n_channels,
                down_rate  = self.down_rate,
                dims       = self.dims)
        else:
            self.h_upd_cond = nn.Identity()
        ...
        if self.n_channels == self.n_out_channels:
            ...
            self.skip_connection_cond = conv_nd2(
                dims         = self.dims,
                in_channels  = self.n_channels,
                out_channels = self.n_out_channels,
                kernel_size  = self.kernel_size,
    ...
    def forward(self,x,emb,cond=None):
        ...
        if self.updown: # upsample or downsample
            ...
            x = self.x_upd(x)
            if self.updown_cond : 
                in_norm_actv_cond = self.in_layers_cond[:-1]
                in_conv_cond = self.in_layers_cond[-1]
                h_bcond = in_norm_actv_cond(cond) 
                h_bcond = self.h_upd_cond(h_bcond)
                h_bcond = in_conv_cond(h_bcond)
                cond = self.cond_upd(cond)
        else:
            h = self.in_layers(x) # [B x C x ...]
            h_bcond = self.in_layers_cond(cond) # [B x C x ...]
        ...
        if self.use_scale_shift_norm:
            ...
            h = out_norm(h) * (1.0 + scale)+ shift + h_bcond # [B x C x ...] 
        ...
        h_bcond = h_bcond+ self.skip_connection_cond(cond) # [B x C x ...]
        return out, h_bcond # [B x C x ...]
