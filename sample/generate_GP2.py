...
def main():
    ...
    if out_path == '':
        out_path = os.path.join(os.path.dirname(args.model_path),
                                'samples_{}_{}_{}_seed{}'.format(name, niter, all_idx, args.seed))
        if args.text_prompt != '':
    ...
    args.text_prompt = 'a man run fast'
    if args.text_prompt != '':
    ...
    for rep_i in range(1):
        ...
        # if args.guidance_param != 1:
        # model_kwargs['y']['scale'] = torch.ones(args.batch_size, device=dist_util.dev()) * 1.0

        sample_fn = diffusion.p_sample_loop

        sample = sample_fn(
            model,
            (1, model.njoints, model.nfeats, 196),  # BUG FIX
            eval_K_params[all_idx].unsqueeze(0),
            eval_len_param[all_idx].unsqueeze(0),
            clip_denoised=False,
        ...
        plot_3d_motion(out_path+'/eval_result_lens_'+lens_str[all_idx]+'.gif', skeleton, motion, dataset=args.dataset, title='length :'+str(lens_array[all_idx]), fps=20)
